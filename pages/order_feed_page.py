...
class OrderFeed(BasePage):
    ...
    def check_private_orders_in_feed(self, new_user):
        ...
        order_id = Order().create_order(ingredients, token).json()['order']['_id']  # создаем бургер
    ...
    def check_order_counter(self, new_user):
        ...
        token = new_user[0].json()["accessToken"]
        ingredients = Order().create_burger()  # получаем список ингредиентов для бургера
        Order().create_order(ingredients, token)  # создаем бургер
    ...
    def check_order_in_progress(self, new_user):
        ...
        token = new_user[0].json()["accessToken"]
        ingredients = Order().create_burger()  # получаем список ингредиентов для бургера
        order_number = Order().create_order(ingredients, token).json()['order']['number']  # создаем бургер
