...
class TestOrderFeed:
    ...
    def test_check_private_orders_in_feed(self, logged_driver, new_user):
        order_feed = OrderFeed(logged_driver)
    ...
    def test_check_order_counter(self, logged_driver, new_user):
        order_feed = OrderFeed(logged_driver)
        assert order_feed.check_order_counter(new_user)

    @allure.title("Проверка отображения созданного заказа в поле 'В работе'")
    def test_check_order_in_progress(self, logged_driver, new_user):
        order_feed = OrderFeed(logged_driver)
